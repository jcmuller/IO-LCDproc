# This Makefile is for the IO::LCDproc extension to perl.
#
...
#     ABSTRACT_FROM => q[LCDproc.pm]
#     AUTHOR => q[Juan C. Muller <jcmuller@gmail.com>]
#     NAME => q[IO::LCDproc]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[LCDproc.pm]
...
TO_INST_PM = LCDproc.pm

PM_TO_BLIB = LCDproc.pm \
	$(INST_LIB)/IO/LCDproc.pm
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  LCDproc.pm $(INST_MAN3DIR)/IO::LCDproc.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         IO-LCDproc' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.034' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: LCDproc.pm' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,034,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Perl extension to connect to an LCDproc ready display.</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  LCDproc.pm $(INST_LIB)/IO/LCDproc.pm 
